  ...
  <div class="calendar">
    ...
    <ul class="calendar-num clearfix">
      <li v-for="(item,idx) of dateArr" :key="idx" @click="queryCheckByDate(item.day, idx)">
        ...
        <span
          :class="[{'cirle': item.num===index}, {'gray': !item.isSchedul}, {'bottom_point': item.isPint && item.state}, {'abnormal_point': item.isPint && !item.state}]"
...
export default {
  ...
  methods: {
    ...
    creatTable(index) {
      ...
      this.studentId = this.$route.query.studentId || 0
      let month = parseInt(this.currentMonth + 1) < 10 ? '0' + parseInt(this.currentMonth + 1).toString() : parseInt(this.currentMonth + 1).toString()
      this.$http
        .get(this.$api.getCheckDateListByStu + this.studentId + '/' + this.classId + '/' + this.currentYear + '-' + month)
        .then(res => {
          ...
          pintArr.forEach((item, index) => {
            dayArr.push(item.checkdate)
          })
          ...
          arr.forEach((item, index) => {
            if (item.num < 10) {
              ...
              if (dayArr.indexOf(v) > -1) {
                arr[index].isPint = true
                // 判断正常还是异常
                let obj = pintArr.filter(item => {
                  return parseInt(item.checkdate) === index + 1 && item.checkstate === 1
                })
                if (obj.length > 0) {
