<template>
  <el-dialog
    ...
    :title="title"
  >
    <el-form class="form" :rules="rules" :model="teacherInfo" ref="form" status-icon :size="size">
      <!--- 基础信息 --->
      ...
      <el-row>
        <el-col :span="24">
          <UploadSingleFile ref="multiUpload" v-model="teacherInfo.headImage" v-bind="upLoadConfig"></UploadSingleFile>
        </el-col>
    ...
    <span slot="footer" class="dialog-footer">
      <el-button
        type="primary"
        @click="$refs.form.resetFields();$refs.multiUpload.clearFiles();"
      >重 置</el-button>
  ...
  data() {
    return {
      ...
      headUpload: {
        ...
        uploadUrl: this.$api.uploadImage + '?folder=teacher'
      },
      rules: {
        name: [
    ...
    submit() {
      this.$refs['form'].validate(valid => {
        ...
        let params = {
          ...
          //photoUrl: this.teacherInfo.headImage.response.message // 头像
        }
        if (this.teacherInfo.headImage.response) {
          params.photoUrl = this.teacherInfo.headImage.response.message
        } else {
          params.photoUrl = ''
        }
        return this.axios
          .post(this.$api.addNewTeacher, params)
          .then(res => {
